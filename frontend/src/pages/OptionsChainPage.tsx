...
import { classifyExpiries } from '../utils/optionsUtils'
import OptionLtpChart from '../components/Options/OptionLtpChart'
...
export default function OptionsChainPage() {
  ...
  // Classify expiries into weekly / monthly
  const { weekly, monthly } = useMemo(
    () => classifyExpiries(data?.expiry_dates ?? []),
    [data?.expiry_dates]
  )
  const visibleExpiries = tabType === 'WEEKLY' ? weekly : monthly

  // Auto-select first expiry when tab or data changes
  useEffect(() => {
    if (visibleExpiries.length) setExpiry(visibleExpiries[0])
  }, [tabType, visibleExpiries.join(',')])
  ...
  const openChart = (row: OptionRow, optionType: 'CE' | 'PE') => {
    const ltp = optionType === 'CE' ? row.CE?.ltp : row.PE?.ltp
    setChartSel({ strike: row.strike, optionType, expiry: row.expiry, initialLtp: ltp || 0 })
  }
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex flex-wrap items-center gap-2 px-3 py-2 bg-[#161b22] border-b border-[#30363d] shrink-0">
        ...
        <div className="flex gap-1">
          {INDICES.map(s => (
            <button key={s} onClick={() => { setSymbol(s); setChartSel(null) }}
        ...
        {/* Weekly / Monthly tabs */}
        <div className="flex gap-0.5 rounded bg-[#1c2128] p-0.5">
          {(['WEEKLY', 'MONTHLY'] as const).map(t => (
            <button key={t} onClick={() => setTabType(t)}
              className={clsx('px-3 py-1 rounded text-xs font-medium transition-colors',
                tabType === t ? 'bg-blue-500 text-white' : 'text-[#8b949e] hover:text-white'
              )}>
              {t}
            </button>
          ))}
      ...
      {!error && (
        <div className="flex flex-1 overflow-hidden">
          {/* Options table */}
          <div className={clsx('overflow-auto', chartSel ? 'flex-1 min-w-0' : 'w-full')}>
            <table className="w-full text-xs">
              ...
              <tbody>
                {rows.map((row) => {
                  ...
                  const isSelected = chartSel?.strike === row.strike
                  return (
                    <tr key={row.strike} className={clsx(
                      'border-b border-[#30363d]/40 cursor-pointer',
                      isATM && 'bg-blue-500/5 border-blue-500/20',
                      isMaxPain && 'bg-yellow-500/5',
                      isSelected && 'ring-1 ring-inset ring-blue-500/50',
                      !isSelected && 'hover:bg-[#1c2128]/50',
                    )}>
                      {/* CE side — click opens CE chart */}
                      ...
                      <td onClick={() => openChart(row, 'CE')}
                        className="px-2 py-1.5 text-right text-green-400 font-mono font-semibold hover:text-green-300 hover:underline select-none">
                      ...
                      {/* PE side — click opens PE chart */}
                      <td className="px-2 py-1.5 text-left text-[#8b949e]">{row.PE?.delta?.toFixed(3)}</td>
                      <td onClick={() => openChart(row, 'PE')}
                        className="px-2 py-1.5 text-left text-red-400 font-mono font-semibold hover:text-red-300 hover:underline select-none">
          ...
          {/* Live LTP chart panel */}
          {chartSel && (
            <div className="w-80 shrink-0 flex flex-col">
              <OptionLtpChart
                underlying={symbol}
                strike={chartSel.strike}
                optionType={chartSel.optionType}
                expiry={chartSel.expiry}
                initialLtp={chartSel.initialLtp}
                onClose={() => setChartSel(null)}
              />
            </div>
          )}
        </div>
